...
class ConsumptionProcessor(Processor):
    ...
    def do_process(self):
        ...
        if all(k in self._df for k in ("start", "value_kwh")):
            ...
            for opt in [
                {"period": "M", "dictkey": "monthly"},
                {"period": "D", "dictkey": "daily"},
            ]:
                _t = self._df.copy()
                for p in ["p1", "p2", "p3"]:
        ...
        if all(k in c_df for k in ("start", "value_kwh")):
            ...
            if all(k in p_df for k in ("start", "value_eur")):
                ...
                try:
                    ...
                    df["total_eur"] = (
                        (df["power_eur"] + df["energy_eur"])
                        * self.const["electricity_tax"]
                        * self.const["iva_tax"]  # power + energy terms
                    ) + (
                        # other (meter) terms
                        df["other_eur"]
                        * self.const["iva_tax"]
                    )
                    df["taxes_eur"] = df["total_eur"] - (
                        df["energy_eur"] + df["power_eur"] + df["other_eur"]
                    )
                    for opt in [
                        {"period": "M", "dictkey": "monthly"},
                        {"period": "D", "dictkey": "daily"},
