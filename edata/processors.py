...
class ConsumptionProcessor:
    ...
    def get_stats (self, dt_from, dt_to):
        if self.valid_data:
            ...
            self.data['p1_kWh'] = _t['value_kWh'].loc[(_t['datetime'].dt.hour.isin(HOURS_P1)) & (~_t['weekday'].isin(DAYS_P3))].sum()
            self.data['p2_kWh'] = _t['value_kWh'].loc[(_t['datetime'].dt.hour.isin(HOURS_P2)) & (~_t['weekday'].isin(DAYS_P3))].sum()
