import logging, requests, jwt, logging, json
...
class DatadisConnector (Connector):
    ...
    def _send_cmd (self, url, data={}, refresh_token=False):
        ...
        if is_valid_token or not refresh_token:
            ...
            if (r.status_code == 200 and r.json()):
                _LOGGER.debug (f"{self._LABEL}: got a valid response for {url + params}")
                response = r.json()
    ...
    def update (self, cups, date_from=datetime(1970, 1, 1), date_to=datetime.today(), ignore_interval=False):
        _LOGGER.info (f"{self._LABEL}: update requested for CUPS {cups[-4:]} from {date_from} to {date_to}")
        ...
        if oldest_contract != s_start and oldest_contract > max ([date_from, s_start]):
            for gap in [x for x in miss_cons if (x['from'] < oldest_contract) and not (x['to'] < s_start)]:
                start = max ([gap['from'], s_start])
                end = gap['to']
                _LOGGER.warning (f'{self._LABEL}: it seems that oldest contract does not match with supply start date, requesting consumptions update from {start} to {end}')
                self._update_consumptions (cups,  dcode,  start,  end,  "0", ptype)
            self._update_maximeter (cups, dcode, max ([date_from, s_start]), oldest_contract)

        if len(miss_cons) > 1:
            self._retries += 1
            if not ignore_interval:
                _LOGGER.info (f"{self._LABEL}: still missing the following consumption ranges {miss_cons}, retrying...")
    ...
    def _update_supplies (self):
        ...
        if len (supplies) > 0:
            ...
            _LOGGER.info (f"{self._LABEL}: supplies data has been successfully updated ({len(supplies)} elements)")
        else:
    ...
    def _update_contracts (self, cups, distributorCode, authorizedNif=None):
        ...
        if len (contracts) > 0:
            ...
            _LOGGER.info (f"{self._LABEL}: contracts data has been successfully updated ({len(contracts)} elements)")
        else:
    ...
    def _update_consumptions (self, cups, distributorCode, startDate, endDate, measurementType, pointType, authorizedNif=None):
        if self._retries >= self.SECURE_FETCH_THRESHOLD and (endDate - startDate).days > 31:
            ...
            for m in mlist:
                self._update_consumptions (cups, distributorCode, max(m, startDate), min(m+relativedelta(months=1), endDate), measurementType, pointType, authorizedNif)
        else:
            r = self.get_consumption_data (cups, distributorCode, startDate, endDate, measurementType, pointType, authorizedNif)
            if len (r) > 0:
                ...
                _LOGGER.info (f"{self._LABEL}: consumptions data has been successfully updated ({len(r)} elements)")
            else:
                self._status['consumptions'] = False
                _LOGGER.debug (f"{self._LABEL}: consumptions data was not updated")
    ...
    def _update_maximeter (self, cups, distributorCode, startDate, endDate, authorizedNif=None):
        ...
        if len (r) > 0:
            ...
            _LOGGER.info (f"{self._LABEL}: maximeter data has been successfully updated ({len(r)} elements)")
        else:
