...
from edata.const import (
    ERROR_CUPS, ERROR_LOGIN_AUTH, ERROR_SERVER_EMPTY, ERROR_SERVER_FAILURE, ERROR_TIMEOUT, anonymize
)
...
class DatadisConnector (Connector):
    ...
    def __init__(self, username, password):
        ...
        self._retries = 0
        self._token = {}
        self._data = DatadisData (supplies = [], contracts = [], consumptions = [], maximeter = [])
        self._last_try = datetime(1970, 1, 1)  
        self._last_updated = {
            x: datetime (1970, 1, 1) for x in self._data
        }

    def login (self):
        is_valid_token = False
        self._session = requests.Session()
        credentials = {"username": self._usr.encode("utf-8"), "password": self._pwd.encode("utf-8")}
        ...
        if (r.status_code == 200):
            ...
            raise ServerError (self._LABEL, ERROR_SERVER_FAILURE (user=self._usr, query=query, response=r.text))
    ...
    def _send_cmd (self, url, data={}, refresh_token=False, is_retry=False):
        ...
        try:
            ...
            if is_valid_token or not refresh_token:
                ...
                # query
                query = url + params
                _LOGGER.info (f"{self._LABEL}: {'running' if not is_retry else 'retrying'} command {url.split('/')[-1]}{params}...")
                try:
                    r = self._session.get(query, timeout=30)
                except requests.exceptions.ReadTimeout as e:
                    raise TimeoutError (self._LABEL, ERROR_TIMEOUT (seconds=30, user=self._usr, query=query))
                # eval response
                if (r.status_code == 200 and r.json()):
                    ...
                    raise ServerError (self._LABEL, ERROR_SERVER_EMPTY (user=self._usr, query=query, response=r.text))
                else:
                    raise ServerError (self._LABEL, ERROR_SERVER_FAILURE (user=self._usr, query=query, response=r.text))
    ...
    def update (self, cups, date_from=None, date_to=None, ignore_interval=False):
        _LOGGER.info (f"{self._LABEL}: update requested for CUPS {anonymize(cups)} from {date_from} to {date_to}")
        ...
        for s in self._data['supplies']:
            ...
            raise CUPSError (self._LABEL, ERROR_CUPS (cups=cups, cups_list=', '.join([anonymize(x["cups"]) for x in self._data['supplies']])))
        ...
        def fill_gaps ():
            ...
            for _contract in self._data['contracts']:
                ...
                for gap in [x for x in miss_cons if not (x['to'] < _contract['start'] or x['from'] > _contract['end'])]:
                    start = max ([gap['from'], _contract['start']])
                    end = min ([gap['to'], _contract['end']])
                    self._update_consumptions (cups,  _supply["distributor_code"],  start,  end,  "0", _supply["point_type"])
                
                # update maximeter during contract range
                for gap in [x for x in miss_maxim if not (x['to'] < _contract['start'] or x['from'] > _contract['end'])]:
                    start = max ([gap['from'], _contract['start'] + relativedelta(months=1)])
                    end = min ([gap['to'], _contract['end']])
                    self._update_maximeter (cups, _supply["distributor_code"], start, end)

            ## safe check periods in wrongly-registered contracts, this might throw errors
            if _supply["start"] < date_from < _start_contracts:
                self._update_consumptions (cups,  _supply["distributor_code"],  max ([gap['from'], _contract['start']]),  _start_contracts,  "0", _supply["point_type"])
                self._update_maximeter (cups, _supply["distributor_code"], date_from, _start_contracts)
        ...
        if len(miss_cons) > 1 or len(miss_maxim) > 1:
            fill_gaps ()
            if len(miss_cons) > 1:
                _LOGGER.warning (f"{self._LABEL}: still missing the following consumption ranges {miss_cons}, will try again later")
            if len(miss_maxim) > 1:
                _LOGGER.warning (f"{self._LABEL}: still missing the following maximeter ranges {miss_maxim}, will try again later")
        ...
        if (datetime.today().date() != self._last_updated['supplies'].date()):
            try:
                ...
                if len(self._data["supplies"]) > 0:
                    _LOGGER.exception ("got an error while updating supplies %s:%s", e.source, e.message)
                else:
                    raise e
    ...
    def _update_contracts (self, cups, distributor_code, authorizedNif=None):
        if (datetime.today().date() != self._last_updated['contracts'].date()):
            try:
                self._data["contracts"] = self.get_contract_detail (cups, distributor_code)
                self._last_updated['contracts'] = datetime.now()
            except (ServerError, TimeoutError) as e:
                if len(self._data["contracts"]) > 0:
                    _LOGGER.exception ("got an error while updating contracts %s:%s", e.source, e.message)
                else:
                    raise e

    def _update_consumptions (self, cups, distributor_code, startDate, endDate, measurementType, point_type, authorizedNif=None):
        try:
            self._data['consumptions'] = add_or_update (
                self._data['consumptions'], 
                self.get_consumption_data (cups, distributor_code, startDate, endDate, measurementType, point_type, authorizedNif), 
                "start"
                )
    ...
    def _update_maximeter (self, cups, distributor_code, startDate, endDate, authorizedNif=None):
        try:
        ...
        for i in r:
            if all (k in i for k in ("cups", "validDateFrom", "validDateTo", 'pointType', 'distributorCode')):
                c.append (
                    Supply (
                        cups = i['cups'],
                        start = datetime.strptime (i['validDateFrom'] if i['validDateFrom'] != '' else '1970/01/01', '%Y/%m/%d'),
                        end = datetime.strptime (i['validDateTo'] if i['validDateTo'] != '' else tomorrow_str, '%Y/%m/%d'),
                ...
                _LOGGER.warning (f'{self._LABEL}: weird data structure while fetching supplies data, got {r}')
        return c

    def get_contract_detail (self, cups, distributor_code, authorizedNif=None):
        data = {
            'cups': cups, 
            'distributorCode': str(distributor_code)
            }
        if authorizedNif is not None:
            data['authorizedNif'] = authorizedNif
        ...
        tomorrow_str = (datetime.today() + timedelta (days=1)).strftime('%Y/%m/%d')
        for i in r:
            if all (k in i for k in ("cups", "startDate", "endDate", "marketer", "contractedPowerkW")):
                _date_template = '%Y/%m/%d'
                c.append (
                    Contract (
                        cups = i['cups'],
                        start = datetime.strptime (i['startDate'] if i['startDate'] != '' else '1970/01/01', _date_template),
                        end = datetime.strptime (i['endDate'] if i['endDate'] != '' else tomorrow_str, _date_template),
                        marketer = i['marketer'],
                        ...
                        power_p3 = i['contractedPowerkW'][2] if (len( i['contractedPowerkW']) > 2) else None,
                        power_p4 = i['contractedPowerkW'][3] if (len( i['contractedPowerkW']) > 3) else None,
                        power_p5 = i['contractedPowerkW'][4] if (len( i['contractedPowerkW']) > 4) else None,
                        power_p6 = i['contractedPowerkW'][5] if (len( i['contractedPowerkW']) > 5) else None,
                    )
                )
            else:
                _LOGGER.warning (f'{self._LABEL}: weird data structure while fetching contracts data, got {r}')
        return c

    def get_consumption_data (self, cups, distributor_code, startDate, endDate, measurementType, point_type, authorizedNif=None):
        ...
        for i in r:
            if i.get('consumptionKWh', 0) > 0:
                if all (k in i for k in ("time", "date", "consumptionKWh", "obtainMethod")):
                    ...
                    _start = datetime.strptime (f"{i['date']} {hour.zfill(2)}:00", '%Y/%m/%d %H:%M')
                    c.append (
                        Consumption (
                            start = _start,
                    ...
                    _LOGGER.warning (f'{self._LABEL}: weird data structure while fetching consumption data, got {r}')
        return c

    def get_max_power (self, cups, distributor_code, startDate, endDate, authorizedNif=None):
            ...
            if all (k in i for k in ("time", "date", "maxPower")):
                ...
                c.append (
                    MaxPower (
                        ...
                        end = _time.replace (day=1, hour=0, minute=0) + relativedelta (months=1),
                        time = _time,
                        value_kw = i['maxPower'],
                    )
                )
            else:
                _LOGGER.warning (f'{self._LABEL}: weird data structure while fetching maximeter data, got {r}')
        return c
...
class EsiosConnector (Connector):
    ...
    def __init__ (self, local_timezone='Europe/Madrid'):
        ...
        self._handler = PVPCData(tariff=TARIFFS[0], local_timezone=tz.timezone(self._local_timezone))
    ...
    def update (self, date_from=None, date_to=None):
        ...
        if (datetime.now() - self._last_try) > self.UPDATE_INTERVAL:
            try:
                self._data["energy_costs"], missing = find_gaps (self._data["energy_costs"], date_from, date_to, gap_interval=timedelta(hours=1))
                for gap in missing:
                    raw = self._handler.download_prices_for_range(gap['from'], gap['to'])
                    pvpc = [
                        Cost (
                            start= datetime.strptime(x.astimezone(tz.timezone(self._local_timezone)).strftime('%Y-%m-%d %H:%M'), '%Y-%m-%d %H:%M'),
                            end= datetime.strptime(x.astimezone(tz.timezone(self._local_timezone)).strftime('%Y-%m-%d %H:%M'), '%Y-%m-%d %H:%M') + timedelta (hours=1),
                            value_eur= raw[x],
                        ) for x in raw
                    ]
                    self._data['energy_costs'] = add_or_update (self._data['energy_costs'], pvpc, "start")
            except Exception as e:
                raise ServerError (self._LABEL, "unhandled error when fetching pvpc data %s", e)
